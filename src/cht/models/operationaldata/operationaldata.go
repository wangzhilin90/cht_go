...
func GetThirtyDaysResult(odrs *OperationalDataRequestStruct) ([]ThirtyDaysResult, error) {
	...
	err := utils.GetCache("borrow:tender:thirtyDays", &tdr)
	if err != nil {
		o := orm.NewOrm()
		o.Using("default")
		buf := bytes.Buffer{}
		...
		utils.SetCache("borrow:tender:thirtyDays", tdr, int(ONE_DAT_TIME))
	}
...
func GetTwelveMonthResult(odrs *OperationalDataRequestStruct) ([]TwelveMonthResult, error) {
	...
	err := utils.GetCache("borrow:twelveMonth", &tmr)
	if err != nil {
		o := orm.NewOrm()
		o.Using("default")
		buf := bytes.Buffer{}
		...
		utils.SetCache("borrow:twelveMonth", tmr, int(ONE_DAT_TIME))
	}
...
func GetOneMonthResult(odrs *OperationalDataRequestStruct) ([]OneMonthResult, error) {
	...
	err := utils.GetCache("borrow:oneMonth", &omrs)
	if err != nil {
		o := orm.NewOrm()
		o.Using("default")
		buf := bytes.Buffer{}
		...
		utils.SetCache("borrow:oneMonth", omrs, int(ONE_DAT_TIME))
	}
...
func GetPeriodResult(odrs *OperationalDataRequestStruct) ([]PeriodResult, error) {
	...
	err := utils.GetCache("borrow:period", &pr)
	if err != nil {
		o := orm.NewOrm()
		o.Using("default")
		buf := bytes.Buffer{}
		...
		utils.SetCache("borrow:period", pr, int(ONE_DAT_TIME))
	}
...
func GetInvestResult(odrs *OperationalDataRequestStruct) (*InvestResult, error) {
	...
	var ir InvestResult
	err := utils.GetCache("borrow:tender:invest", &ir)
	if err != nil {
		o := orm.NewOrm()
		o.Using("default")
		buf := bytes.Buffer{}
		...
		utils.SetCache("borrow:tender:invest", ir, int(ONE_DAT_TIME))
	}
...
func GetBidResult(odrs *OperationalDataRequestStruct) ([]BidResult, error) {
	...
	var br []BidResult
	err := utils.GetCache("borrow:bid", &br)
	if err != nil {
		o := orm.NewOrm()
		o.Using("default")
		buf := bytes.Buffer{}
		...
		utils.SetCache("borrow:bid", br, int(ONE_DAT_TIME))
	}
...
func GetWaitResult(odrs *OperationalDataRequestStruct) ([]WaitResult, error) {
	...
	var wr []WaitResult
	err := utils.GetCache("user:wait", &wr)
	if err != nil {
		o := orm.NewOrm()
		o.Using("default")
		buf := bytes.Buffer{}
		...
		utils.SetCache("user:wait", wr, int(ONE_DAT_TIME))
	}
...
func GetTwelveMonthTotalNum(odrs *OperationalDataRequestStruct) (string, error) {
	...
	var totalNum string
	err := utils.GetCache("borrow:twelveMonth:account", &totalNum)
	if err != nil {
		o := orm.NewOrm()
		o.Using("default")
		buf := bytes.Buffer{}
		...
		utils.SetCache("borrow:twelveMonth:account", totalNum, int(ONE_DAT_TIME))
	}
...
func GetTotalRepayment(odrs *OperationalDataRequestStruct) (string, error) {
	...
	err := utils.GetCache("borrow:repayment:money", &totalRepayment)
	if err != nil {
		o := orm.NewOrm()
		o.Using("default")
		buf := bytes.Buffer{}
		...
		utils.SetCache("borrow:repayment:money", totalRepayment, int(ONE_DAT_TIME))
	}
...
func GetTender(odrs *OperationalDataRequestStruct) (string, error) {
	...
	if err != nil {
		...
		utils.SetCache("borrow:tender", tender, int(ONE_DAT_TIME))
	}
...
func GetTenderToday(odrs *OperationalDataRequestStruct) (string, error) {
	...
	if err != nil {
		...
		utils.SetCache("borrow:account", tenderToday, int(ONE_DAT_TIME))
	}
...
func GetProfit(odrs *OperationalDataRequestStruct) (string, error) {
	...
	if err != nil {
		...
		utils.SetCache("borrow:collection:interest", profit, int(ONE_DAT_TIME))
	}
...
func GetTenderUserCount(odrs *OperationalDataRequestStruct) (int32, error) {
	...
	err := utils.GetCache("borrow:tender:user:count", &tenderUserCount)
	if err != nil {
		o := orm.NewOrm()
		o.Using("default")
		buf := bytes.Buffer{}
		...
		utils.SetCache("borrow:tender:user:count", tenderUserCount, int(ONE_DAT_TIME))
	}
